...
class TestIntegration(unittest.TestCase):
    def setUp(self):
        ...
        # Set up mock guests
        self.dr_yan_nitor = Guest("Dr.Yan Nitor", 10000, self.rickRoll)
    ...
    def test_wallets(self):
        ...
        self.assertEqual(9900, self.dr_yan_nitor.wallet)
    ...
    def test_vip_gets_discount(self):
        ...
        self.assertEqual(9805, self.dr_yan_nitor.wallet)
    ...
    def test_loyalty_status(self):
        ...
        for _ in range(11):
            self.ben_dover.enter_room(self.room1)
            self.ben_dover.leave_room()
        self.assertEqual("Bronze", self.room1.check_loyalty_status(self.ben_dover))
        
        for _ in range(51):
            self.shanda_leer.enter_room(self.room1)
            self.shanda_leer.leave_room()
        self.assertEqual("Silver", self.room1.check_loyalty_status(self.shanda_leer))
        
        for _ in range(101):
            self.dr_yan_nitor.enter_room(self.room1)
            self.dr_yan_nitor.leave_room()
        self.assertEqual("Gold", self.room1.check_loyalty_status(self.dr_yan_nitor))